	...
	<tr>
		<td
			...
			<c:choose>
					...
					<c:choose>
						...
						<c:otherwise>
							<span class='current'> <a href='#'
								onclick="toPage(${PageBean.pageNow+1})">下一页</a></span>
						</c:otherwise>
					</c:choose>
				</c:when>
				<c:when test="${PageBean.pageNow==PageBean.pageCount}">
					<span class='current'><a href='#'
						onclick="toPage(${PageBean.pageNow-1})">上一页</a></span>
					<c:forEach begin="1" end="${PageBean.pageCount}" var="i">
						<c:choose>
							<c:when test="${PageBean.pageNow==i}">
								<span class='current'>${i}</span>
				...
				<c:otherwise>
					<span class='current'><a href='#' onclick="toPage(${PageBean.pageNow-1})" >上一页</a></span>
					<c:forEach begin="1" end="${PageBean.pageCount}" var="i">
						<c:choose>
							<c:when test="${PageBean.pageNow==i}">
								<span class='current'>${i}</span>
